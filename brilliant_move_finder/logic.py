...
def analyse(engine: chess.engine.SimpleEngine, board: chess.Board, depth: int, multipv: int = 1) -> list[dict]:
    info = engine.analyse(
        board,
...
def best_line_children(
    ...
    infos = analyse(engine, board, settings.shallow_depth, multipv=max(settings.frontier_width, settings.multipv))
...
def find_brilliant_moves(
    ...
    def evaluate_node(node: chess.Board, path: list[chess.Move], ply: int) -> None:
        ...
        root_infos = analyse(engine, node, settings.root_depth, multipv=max(settings.frontier_width, settings.multipv))
        if not root_infos:
            return

        best_entry = root_infos[0]
        best_pv = best_entry.get("pv") or []
        if best_pv:
            best_move = best_pv[0]
            ...
            if best_move in node.legal_moves:
                ...
                if profile_after.is_real_sacrifice and not profile_after.is_free_capture and not profile_after.is_defensive_only:
                    shallow_infos = analyse(engine, after, settings.shallow_depth, multipv=1)
                    deep_infos = analyse(engine, after, settings.root_depth, multipv=1)
                    ...
                    root_cp = cp_from_score(best_entry["score"], node.turn)

                    reply_infos = analyse(engine, after, settings.reply_depth, multipv=min(3, max(1, settings.frontier_width)))
                    best_acceptance_san = ""
                    best_decline_san = ""
                    best_defense_san = ""
                    ...
                    for reply_info in reply_infos:
                        ...
                        cont_infos = analyse(engine, reply_board, settings.continuation_depth, multipv=1)
                        if not cont_infos:
                            continue
        ...
        for child_move, _score in best_line_children(engine, node, settings, cancel_event):
            assert_active(cancel_event)
